...
static void ImGui_ImplVulkan_CreatePipeline(VkDevice device, const VkAllocationCallbacks* allocator, VkPipelineCache pipelineCache, VkFormat colorFormat, VkFormat depthFormat, VkSampleCountFlagBits MSAASamples, VkPipeline* pipeline, uint32_t subpass)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    if (!bd->DescriptorSetLayout)
    {
        ...
        check_vk_result(err);
        bd->DescriptorSetLayoutBindings = binding[0];
    }
    ...
    ImGui_ImplVulkan_CreatePipeline(v->Device, v->Allocator, v->PipelineCache, bd->Format, bd->DepthFormat, v->MSAASamples, &bd->Pipeline, bd->Subpass);
...
bool    ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info, VkFormat format, VkFormat depthFormat)
{
    ...
    IM_ASSERT(format != VK_FORMAT_UNDEFINED);
    IM_ASSERT(depthFormat != VK_FORMAT_UNDEFINED);

    bd->VulkanInitInfo = *info;
    bd->Format = format;
    bd->DepthFormat = depthFormat;
...
static void ImGui_ImplVulkan_RenderWindow(ImGuiViewport* viewport, void*)
{
    ...
    ImGui_ImplVulkanH_FrameSemaphores* fsd = &wd->FrameSemaphores[wd->SemaphoreIndex];
    {
        ...
        }
        {
            ...
            renderingInfo.renderArea = { {}, { (uint32_t)wd->Width, (uint32_t)wd->Height } };
